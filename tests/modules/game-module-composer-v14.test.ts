...
describe("Graph 1.4 compositor", () => {
  it("retains the complete inherited player attack projection unchanged", async () => {
    const registry = await createBatch3ProductionRegistry();
    const inherited = resolveGameAssemblyV13(
      BATCH2_VERTICAL_SLICE_ASSEMBLY,
      registry,
    ).graph;
    const assembly = GameAssemblySpecV13Schema.parse({
      ...structuredClone(BATCH2_VERTICAL_SLICE_ASSEMBLY),
      schemaVersion: "1.3.0",
      actorRootBindings: [],
    ...
    expect(result.graph.attackChannels).toEqual(inherited.attackChannels);
    expect(result.graph.projectileChannelLineages).toEqual(
      inherited.projectileChannelLineages,
    );
    expect(result.graph.projectileBudgetContention).toEqual(
      inherited.projectileBudgetContention,
    );
    expect(result.graph.attackChannels).not.toHaveLength(0);
    expect(
      result.graph.projectileBudgetContention.orderedOwners,
    ).not.toHaveLength(0);
  });
