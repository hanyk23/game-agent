...
describe("ADR 0028 strict contracts", () => {
  ...
  it("allows cross-owner actor-root inputs without inventing an entity role", () => {
    const inputPort = {
      id: "roots",
      payloadType: "actor-root-channel-v1",
      required: true,
    ...
    const manifest = v14Manifest({
      moduleId: "interaction.actor-root-test",
      implementationId: "interaction.actor-root-test.v1",
      configurationSchemaId: "interaction.actor-root-test.config",
    ...
    expect(() =>
      GameModuleManifestV14Schema.parse({
        ...manifest,
        inputPorts: [
          { ...inputPort, payloadType: "damage-v2", delivery: "event" },
        ],
        actorRootConsumers: [],
        actorRootContactConsumer: null,
      }),
