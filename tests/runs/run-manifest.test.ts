...
describe("run manifest", () => {
  ...
  it("accepts only the complete Batch 3 evidence boundary in manifest 1.6.0", () => {
    const manifest = createRunManifest("Generate a modular game", {
      runId,
      now: () => start,
    });
    const evidence = (path: string, value: string) => ({
    ...
    const modular = {
      ...
      artifacts: {
        gameAssemblySpec: evidence("game-assembly-spec.json", "a"),
        scoreEvidence: evidence("score-evidence.json", "b"),
        moduleResolution: evidence("module-resolution.json", "c"),
        resolvedModuleGraph: evidence("resolved-module-graph.json", "d"),
        moduleCatalog: evidence("module-catalog.json", "e"),
      },
    } as const;

    expect(RunManifestSchema.parse(modular)).toMatchObject({
      manifestVersion: "1.6.0",
      composition: { mode: "batch3-legacy-v14" },
      artifacts: {
        gameAssemblySpec: { path: "game-assembly-spec.json" },
        moduleCatalog: { path: "module-catalog.json" },
      },
    });
    expect(
      RunManifestSchema.safeParse({
        ...modular,
        artifacts: { ...modular.artifacts, moduleCatalog: undefined },
      }).success,
    ).toBe(false);
  });

  it("rejects modular mode or evidence on manifest 1.1.0 through 1.5.0", () => {
    const manifest = createRunManifest("Generate a fixed game", {
      runId,
      now: () => start,
    });
    const modularComposition = {
      mode: "batch3-legacy-v14" as const,
  ...
  it("keeps fixed manifest creation on byte-compatible version 1.5.0", () => {
    const manifest = createRunManifest("Generate a game", {
      runId,
      now: () => start,
    });
    expect(manifest.manifestVersion).toBe("1.5.0");
    expect(manifest.composition).toBeUndefined();
