...
- 当前保留版本化 `ShooterGameSpec`、可执行基础模块库和通过验证的 Phaser 模板作为迁移基线；新主线按 ADR 0029 将薄运行内核移植到 Cocos Creator Web/H5。具体 Cocos 版本须经兼容性 spike 后锁定，模型编排和 OpenCode 隔离开发控制器仍属于后续阶段。
