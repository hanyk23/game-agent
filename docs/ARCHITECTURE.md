...
Natural-language request + reviewed module catalog
  ...
  -> thin Phaser runtime kernel + reviewed gameplay modules
...
ADR 0022 replaces fixed-template expansion with a thin Phaser kernel plus
versioned gameplay modules. ADR 0023 refines those modules into orthogonal
...
deterministic core services, and one Phaser adapter. The preserved `PlayScene`
uses those services without moving gameplay rules. Version selectors are
