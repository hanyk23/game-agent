...
Cocos-targeted runtime kernel, an executable base-module library, data-only
...
Build a deterministic TypeScript generation pipeline that uses OpenCode through its supported SDK as a permission-bounded code agent, composes a pinned reviewed Web runtime from a validated specification and licensed asset catalog, and validates the result through Playwright before a maximum-three-round repair loop. ADR 0029 changes the active engine target to Cocos Creator Web/H5 while preserving Phaser as migration evidence.
...
| Game              | Cocos Creator Web/H5, exact version pending bounded spike                | User-authorized target; must prove reproducible browser build and packaging        |
