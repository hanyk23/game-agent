...
export const RunManifestSchema = z
  .strictObject({
    ...
    artifacts: z
      .strictObject({
        ...
        gameAssemblySpec: ArtifactEvidenceSchema.optional(),
        scoreEvidence: ArtifactEvidenceSchema.optional(),
        moduleResolution: ArtifactEvidenceSchema.optional(),
        resolvedModuleGraph: ArtifactEvidenceSchema.optional(),
        moduleCatalog: ArtifactEvidenceSchema.optional(),
    ...
    composition: z
      .strictObject({
        mode: z.literal("batch3-legacy-v14").optional(),
  ...
  .superRefine((manifest, context) => {
    ...
    const moduleArtifactCount = moduleArtifactNames.filter(
      (name) => manifest.artifacts[name] !== undefined,
    ).length;
    const isModularV16 = manifest.manifestVersion === "1.6.0";

    if (isModularV16) {
      if (manifest.composition?.mode !== "batch3-legacy-v14")
        context.addIssue({
          code: "custom",
          message: "manifest 1.6.0 requires batch3-legacy-v14 composition mode",
          path: ["composition", "mode"],
        });
      if (moduleArtifactCount !== moduleArtifactNames.length)
        context.addIssue({
          code: "custom",
          message:
            "manifest 1.6.0 requires the complete Batch 3 module evidence set",
          path: ["artifacts"],
        });
    } else {
      if (manifest.composition?.mode !== undefined)
        context.addIssue({
          code: "custom",
          message: "legacy manifests cannot declare modular composition mode",
          path: ["composition", "mode"],
        });
      if (moduleArtifactCount !== 0)
        context.addIssue({
          code: "custom",
          message: "legacy manifests cannot carry Batch 3 module evidence",
          path: ["artifacts"],
        });
    }
  });
