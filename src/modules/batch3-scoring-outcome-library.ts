...
const defeatManifest = manifest("scoring", {
  ...
  inputPorts: [input("defeats", "defeat-evidence-v1", "event")],
...
const defeatFactory = `export function create(context){const c=context.configuration;let n=0;context.ports.declareHandler("defeats",v=>{if(v.baseScore!==c.scoreBySourceId[v.sourceId])throw new Error("defeat score source mismatch");context.ports.emitEvent("sources",Object.freeze({sequence:n++,emittedAtMs:context.clock.nowMs(),sourceEvidenceId:v.deactivationEvidenceId,kind:"defeat",baseAward:v.baseScore}));});return Object.freeze({instanceId:context.identity.instanceId});}`;
