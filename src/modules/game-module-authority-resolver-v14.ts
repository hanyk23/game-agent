...
export type Batch3AuthorityPlanV14 = Readonly<{
  ...
  actorSetDamageRoutes: readonly Readonly<{
    ...
    defeatLifecycle: Readonly<{
      defeatedOutputPort: string;
      producerInstanceId: string;
      defeatedInputPort: string;
    }> | null;
  }>[];
  ...
  contactRoutes: readonly Readonly<{
    routeId: string;
    kind: "projectile-root" | "actor-root-player";
    detectorInstanceId: string;
    policyInstanceId: string;
    resolutionInstanceId: string;
    sourceChannelProviderInstanceId: string | null;
    rootChannelId: string;
    sourceMutationGrantId: string | null;
    targetDamageRouteId: string;
    targetSinkInstanceId: string;
    sourceOperation: "consume" | "deactivate-root";
  }>[];
...
export function resolveBatch3AuthorityPlanV14(input: {
  ...
  >();
  const resourceGrantByInstance = new Map<string, ModuleResourceBudget>();
  for (const selection of input.manifests) {
    ...
    resourceGrantByInstance.set(
      selection.instanceId,
      selection.resourceGrant ?? manifest.resources,
    );
  }
  ...
  for (const request of assembly.modules) {
    ...
    if (
      ...
      resourceGrantByInstance.get(deliveryBinding.to.instanceId)!
        .activeEntities > group.activeEntityCapacity ||
      resourceGrantByInstance.get(deliveryBinding.to.instanceId)!
        .activeProjectiles > group.activeProjectileCapacity ||
      resourceGrantByInstance.get(deliveryBinding.to.instanceId)!
        .spawnsPerSecond > group.spawnsPerSecondCapacity
    ) {
      addBlocker(
        blockers,
        request.instanceId,
  ...
  for (const route of assembly.actorSetDamageRoutes) {
    ...
    const producerManifest =
      lineage === undefined
        ? undefined
        : manifestByInstance.get(lineage.root.producerInstanceId);
    const productionHealthRoute =
      terminalManifest?.moduleId === "combat.health" &&
      terminalManifest.version === "1.2.0";
    const defeatBinding =
      sink === undefined || sink === null || lineage === undefined
        ? undefined
        : assembly.bindings.find(
            (binding) =>
              binding.from.instanceId === terminalInstanceId &&
              binding.from.portId === sink.defeatedOutputPort &&
              binding.to.instanceId === lineage.root.producerInstanceId &&
              producerManifest?.inputPorts.some(
                (port) =>
                  port.id === binding.to.portId &&
                  port.payloadType === "actor-defeated-v1",
              ),
          );
    if (
      lineage === undefined ||
      ...
      (productionHealthRoute && defeatBinding === undefined)
    ) {
      addBlocker(
        blockers,
    ...
    actorSetDamageRoutes.push({
      ...
      defeatLifecycle:
        defeatBinding === undefined
          ? null
          : {
              defeatedOutputPort: defeatBinding.from.portId,
              producerInstanceId: defeatBinding.to.instanceId,
              defeatedInputPort: defeatBinding.to.portId,
            },
    });
  }
  ...
  const moduleRequestById = new Map(
    assembly.modules.map((module) => [module.instanceId, module] as const),
  );
  const contactRoutes: Batch3AuthorityPlanV14["contactRoutes"][number][] = [];
  for (const [detectorInstanceId, detectorManifest] of manifestByInstance) {
    if (detectorManifest.schemaVersion !== "1.4.0") continue;
    const projectileDescriptor =
      detectorManifest.projectileChannelConsumer?.role === "contact-detector"
        ? detectorManifest.projectileChannelConsumer
        : null;
    const actorRootDescriptor = detectorManifest.actorRootContactConsumer;
    if (projectileDescriptor === null && actorRootDescriptor === null) continue;
    if (
      detectorManifest.moduleId !== "interaction.projectile-root-contact" &&
      detectorManifest.moduleId !== "interaction.actor-root-contact"
    )
      continue;
    const kind =
      projectileDescriptor === null
        ? ("actor-root-player" as const)
        : ("projectile-root" as const);
    const candidateOutputPort =
      projectileDescriptor?.candidateOutputPort ??
      actorRootDescriptor!.candidateOutputPort;
    const policyBindings = assembly.bindings.filter(
      (binding) =>
        binding.from.instanceId === detectorInstanceId &&
        binding.from.portId === candidateOutputPort,
    );
    const policyBinding = policyBindings[0];
    const policyRequest =
      policyBinding === undefined
        ? undefined
        : moduleRequestById.get(policyBinding.to.instanceId);
    const policyManifest =
      policyRequest === undefined
        ? undefined
        : manifestByInstance.get(policyRequest.instanceId);
    const resolutionBindings =
      policyRequest === undefined
        ? []
        : assembly.bindings.filter(
            (binding) =>
              binding.from.instanceId === policyRequest.instanceId &&
              binding.from.portId === "decisions",
          );
    const resolutionBinding = resolutionBindings[0];
    const resolutionRequest =
      resolutionBinding === undefined
        ? undefined
        : moduleRequestById.get(resolutionBinding.to.instanceId);
    const resolutionManifest =
      resolutionRequest === undefined
        ? undefined
        : manifestByInstance.get(resolutionRequest.instanceId);
    const detectorRequest = moduleRequestById.get(detectorInstanceId)!;
    if (
      policyBindings.length !== 1 ||
      policyBinding?.to.portId !== "candidates" ||
      policyManifest?.schemaVersion !== "1.4.0" ||
      policyManifest.moduleId !== "interaction.contact-default-damage" ||
      policyManifest.version !== "1.1.0" ||
      policyRequest?.ownerId !== detectorRequest.ownerId ||
      resolutionBindings.length !== 1 ||
      resolutionBinding?.to.portId !== "decisions" ||
      resolutionManifest?.schemaVersion !== "1.4.0" ||
      resolutionManifest.moduleId !== "interaction.contact-resolution" ||
      resolutionManifest.version !== "1.2.0" ||
      resolutionRequest?.ownerId !== detectorRequest.ownerId
    ) {
      addBlocker(blockers, detectorInstanceId, "invalid-contact-v2-chain");
      continue;
    }
    const policyConfiguration = policyRequest.configuration as Readonly<
      Record<string, unknown>
    >;
    if (kind === "projectile-root") {
      const root = roots.find((candidate) =>
        candidate.consumerGrants.some(
          (grant) =>
            grant.consumerInstanceId === detectorInstanceId &&
            grant.purpose === "projectile-target",
        ),
      );
      const damageRoute = actorSetDamageRoutes.find(
        (route) => route.rootChannelId === root?.rootChannelId,
      );
      const sourceBindings = assembly.bindings.filter(
        (binding) =>
          binding.to.instanceId === detectorInstanceId &&
          binding.to.portId === projectileDescriptor!.sourceChannelInputPort,
      );
      if (
        root === undefined ||
        damageRoute === undefined ||
        sourceBindings.length !== 1 ||
        policyConfiguration.projectileRootRouteId !== damageRoute.routeId
      ) {
        addBlocker(
          blockers,
          detectorInstanceId,
          "invalid-projectile-root-contact-lineage",
        );
        continue;
      }
      contactRoutes.push({
        routeId: `${detectorInstanceId}.contact-v2`,
        kind,
        detectorInstanceId,
        policyInstanceId: policyRequest.instanceId,
        resolutionInstanceId: resolutionRequest.instanceId,
        sourceChannelProviderInstanceId: sourceBindings[0]!.from.instanceId,
        rootChannelId: root.rootChannelId,
        sourceMutationGrantId: null,
        targetDamageRouteId: damageRoute.routeId,
        targetSinkInstanceId: damageRoute.orderedSinkInstanceIds.at(-1)!,
      ...
      const root = roots.find((candidate) =>
        candidate.consumerGrants.some(
          (grant) =>
            grant.consumerInstanceId === detectorInstanceId &&
            grant.purpose === "body-contact",
        ),
      );
      const mutationGrant = actorRootMutationGrants.find(
        (grant) =>
          grant.consumerInstanceId === detectorInstanceId &&
          grant.rootChannelId === root?.rootChannelId,
      );
      const targetDamageRoute = (assembly.damageSinkRoutes ?? []).find(
        (route) => route.ownerId === detectorRequest.ownerId,
      );
      const routeId = policyConfiguration.actorRootPlayerRouteId;
      if (
        root === undefined ||
        mutationGrant === undefined ||
        targetDamageRoute === undefined ||
        typeof routeId !== "string"
      ) {
        addBlocker(
          blockers,
          detectorInstanceId,
          "invalid-actor-root-player-contact-lineage",
        );
        continue;
      }
      contactRoutes.push({
        ...
        rootChannelId: root.rootChannelId,
        sourceMutationGrantId: mutationGrant.grantId,
        targetDamageRouteId: routeId,
        targetSinkInstanceId: targetDamageRoute.headInstanceId,
  ...
  for (const [instanceId, manifest] of manifestByInstance)
    if (
      manifest.schemaVersion === "1.4.0" &&
      (manifest.moduleId === "interaction.contact-default-damage" ||
        manifest.moduleId === "interaction.contact-resolution") &&
      !contactRoutes.some(
        (route) =>
          route.policyInstanceId === instanceId ||
          route.resolutionInstanceId === instanceId,
      )
    )
      addBlocker(blockers, instanceId, "orphan-contact-v2-participant");
  ...
  const plan: Batch3AuthorityPlanV14 = {
    ...
    contactRoutes: contactRoutes.map((route) =>
      ResolvedContactRouteV14Schema.parse(route),
    ),
    scoringAuthority:
