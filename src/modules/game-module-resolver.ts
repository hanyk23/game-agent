...
function buildCapabilityEdges(
  ...
  for (const entry of selected) {
    for (const requirement of entry.registration.manifest.requires) {
      ...
      const manifest = entry.registration.manifest;
      if (
        requirement.id === "delivery.projectile-channel" &&
        requirement.cardinality === "exactly-one" &&
        "projectileChannelConsumer" in manifest &&
        manifest.projectileChannelConsumer !== null
      ) {
        const sourceInputPort =
          manifest.projectileChannelConsumer.sourceChannelInputPort;
        const boundProviderIds = new Set(
          assembly.bindings
            .filter(
              (binding) =>
                binding.to.instanceId === entry.instanceId &&
                binding.to.portId === sourceInputPort,
            )
      ...
      if (
        requirement.id === "interaction.contact-decision-v2" &&
        requirement.cardinality === "exactly-one"
      ) {
        const boundProviderIds = new Set(
          assembly.bindings
            .filter((binding) => binding.to.instanceId === entry.instanceId)
            .map((binding) => binding.from.instanceId),
        );
        matches = matches.filter((provider) =>
          boundProviderIds.has(provider.instanceId),
...
function validateBindings(
  ...
  for (const entry of selected) {
    for (const input of entry.registration.manifest.inputPorts) {
      const hostBoundV14PlayerDamage =
        includeV14 &&
        assembly.schemaVersion === "1.3.0" &&
        (assembly.damageSinkRoutes ?? []).some(
          (route) =>
            route.ownerId === entry.ownerId &&
            route.headInstanceId === entry.instanceId,
        ) &&
        input.payloadType === "damage-v1" &&
        selected.some((candidate) => {
          const manifest = candidate.registration.manifest;
          if (
            candidate.ownerId !== entry.ownerId ||
            ...
            return false;
          const candidateBinding = assembly.bindings.find(
            (binding) =>
              binding.from.instanceId === candidate.instanceId &&
              binding.from.portId ===
                manifest.actorRootContactConsumer!.candidateOutputPort,
          );
          if (candidateBinding === undefined) return false;
          const policy = selected.find(
            (module) => module.instanceId === candidateBinding.to.instanceId,
          );
          if (
            policy?.ownerId !== entry.ownerId ||
            policy.registration.manifest.schemaVersion !== "1.4.0" ||
            policy.registration.manifest.moduleId !==
              "interaction.contact-default-damage" ||
            policy.registration.manifest.version !== "1.1.0"
          )
            return false;
          const resolutionBinding = assembly.bindings.find(
            (binding) =>
              binding.from.instanceId === policy.instanceId &&
              binding.from.portId === "decisions",
          );
          const resolution = selected.find(
            (module) => module.instanceId === resolutionBinding?.to.instanceId,
          );
          return (
            resolution?.ownerId === entry.ownerId &&
            resolution.registration.manifest.schemaVersion === "1.4.0" &&
            resolution.registration.manifest.moduleId ===
              "interaction.contact-resolution" &&
            resolution.registration.manifest.version === "1.2.0"
          );
        });
      if (
        input.required &&
        !targetCounts.has(`${entry.instanceId}.${input.id}`) &&
        !hostBoundV14PlayerDamage
      ) {
        fail(
...
function resolveGameAssemblyInternal(
  ...
  const bindings = validateBindings(assembly, selected, includeV14);
...
export function resolveGameAssemblyV12(
  ...
  const catalogEvidenceId = sha256CanonicalJson(
    modules.map((module) => ({
      instanceId: module.instanceId,
      catalogEntryEvidenceId:
        "catalogEntryEvidenceId" in module
          ? (module.catalogEntryEvidenceId ?? null)
          : null,
    })),
  );
...
type ResolvedInheritedModuleV14 = Readonly<{
  [key: string]: unknown;
  instanceId: string;
  ownerId: string;
  moduleId: string;
...
export type ResolvedInheritedGraphProjectionV14 = Omit<
  ResolvedModuleGraphV13,
  "modules"
> &
  Readonly<{ modules: readonly ResolvedInheritedModuleV14[] }>;

export type ResolvedInheritedGraphProjectionV14Result = Readonly<{
  graph: ResolvedInheritedGraphProjectionV14;
  readinessReport: ExecutionReadinessReport;
}>;

/**
 * Shared ADR 0027 projection. Graph 1.4 uses the inherited V1.3 semantics from
 * this pass and then adds ADR 0028 authority; the public Graph 1.3 entry point
 * below keeps its exact version-isolated wrapper and evidence basis.
 */
export function resolveGameAssemblyInheritedV13Projection(
  assemblyInput: unknown,
  registry: GameModuleRegistry,
  assetEvidence: ModuleAssetAdmissionEvidenceV12 = Object.freeze({
    assets: Object.freeze([]),
  ...
  includeV14 = false,
): ResolvedModuleGraphV13Result | ResolvedInheritedGraphProjectionV14Result {
  const parsed = GameAssemblySpecSchema.safeParse(assemblyInput);
  if (!parsed.success)
    fail(
      ModuleResolutionErrorCode.assemblyInvalid,
  ...
  if (parsed.data.schemaVersion !== (includeV14 ? "1.3.0" : "1.2.0"))
    fail(
      ModuleResolutionErrorCode.incompatibleContractVersion,
      includeV14
        ? "Graph 1.4 inherited projection requires Assembly 1.3"
        : "Graph 1.3 requires Assembly 1.2",
    );
  const assembly = parsed.data;
  const legacy = resolveGameAssemblyInternal(assembly, registry, includeV14);
  const registrationByInstance = new Map(
    assembly.modules.map((request) => {
      ...
      return [
        request.instanceId,
        (includeV14 ? registry.findForGraphV14 : registry.find).call(
          registry,
          module.moduleId,
          module.version,
        )[0]!,
  ...
  const modules = legacy.modules.map((module) => {
    ...
    if (
      ...
      manifest.schemaVersion !== "1.3.0" &&
      (!includeV14 || manifest.schemaVersion !== "1.4.0")
    ) {
      blockers.push({
        instanceId: module.instanceId,
  ...
  for (const module of modules) {
    ...
    if (descriptor.role === "contact-detector") {
      const candidateOutput = manifest.outputPorts.find(
        (port) => port.id === descriptor.candidateOutputPort,
      );
      const usesV14ContactRoute =
        manifest.schemaVersion === "1.4.0" &&
        candidateOutput?.payloadType === "contact-candidate-v2";
  ...
  for (const binding of assembly.effectApplicationBindings) {
    ...
    const commit =
      source?.schemaVersion === "1.3.0" || source?.schemaVersion === "1.4.0"
        ? source.preparedEffectCommit
        : null;
    const targets =
      target?.schemaVersion === "1.3.0" || target?.schemaVersion === "1.4.0"
  ...
  for (const selection of assembly.pickupEffectPlanSelections) {
    ...
    const commitManifestV13 =
      commitManifest?.schemaVersion === "1.3.0" ||
      commitManifest?.schemaVersion === "1.4.0"
        ? commitManifest
        : undefined;
    const commit = commitManifestV13?.preparedEffectCommit ?? null;
    const transform =
      transformManifest?.schemaVersion === "1.3.0" ||
      transformManifest?.schemaVersion === "1.4.0"
  ...
  const graph = deepFreezeData({
    ...
    projectileBudgetContention,
    resourceTotals: legacy.resourceTotals,
  }) as unknown as ResolvedInheritedGraphProjectionV14;
  if (!includeV14)
...
export function resolveGameAssemblyV13(
  ...
  return resolveGameAssemblyInheritedV13Projection(
    assemblyInput,
    registry,
    assetEvidence,
    false,
  ) as ResolvedModuleGraphV13Result;
}
