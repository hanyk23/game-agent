...
const EndpointAuthorizationV14Schema = z
  .strictObject({
    ownerRelation: z.enum(["same-owner", "different-owner"]),
    sourceActorRoles: z.array(ActorRoleSchema).min(1).max(5),
    targetActorRoles: z.array(ActorRoleSchema).min(1).max(5),
...
const InputPortV14Schema = InputPortV11Schema.extend({
  ...
  authorization: EndpointAuthorizationV14Schema,
});
...
export const GameModuleManifestV14Schema = z
  ...
  .superRefine((manifest, context) => {
    ...
    const mapInheritedPayload = <
      T extends {
        payloadType: string;
        authorization?: { ownerRelation: string; sourceEntityRoles: string[] };
      },
    >(
      port: T,
    ) => {
      const mapped = {
        ...port,
        payloadType: PortPayloadTypeV13Schema.safeParse(port.payloadType)
          .success
          ? port.payloadType
          : port.payloadType === "contact-candidate-v2"
            ? "contact-candidate-v1"
            : "movement-vector-v1",
      };
      if (
        (port.payloadType === "actor-root-channel-v1" ||
          port.payloadType === "actor-root-lifecycle-v1" ||
          port.payloadType === "encounter-handoff-v1") &&
        port.authorization?.ownerRelation === "different-owner" &&
        port.authorization.sourceEntityRoles.length === 0
      )
        return {
          ...mapped,
          authorization: {
            ...port.authorization,
            sourceEntityRoles: ["v14-host-event-inherited-diagnostic"],
          },
        };
      return mapped;
    };
    ...
    for (const [index, port] of manifest.inputPorts.entries())
      if (
        port.authorization.ownerRelation === "different-owner" &&
        port.authorization.sourceEntityRoles.length === 0 &&
        !(
          (port.payloadType === "actor-root-channel-v1" &&
            actorRootConsumers.some(
              (consumer) => consumer.rootChannelInputPort === port.id,
            )) ||
          (port.payloadType === "encounter-handoff-v1" &&
            manifest.kind === "encounter-flow") ||
          (port.payloadType === "actor-root-lifecycle-v1" &&
            (manifest.kind === "scoring" || manifest.kind === "outcome"))
        )
      )
        context.addIssue({
          code: "custom",
          message:
            "different-owner authorization without sourceEntityRoles requires an exact actor-root consumer or encounter handoff input",
          path: ["inputPorts", index, "authorization", "sourceEntityRoles"],
        });
