...
function createResolvedPortsV13(
  ...
  isHostBoundRequiredInput: (
    port: ResolvedModuleGraphV13["modules"][number]["runtimePorts"]["inputPorts"][number],
  ) => boolean = () => false,
  ...
  return Object.freeze({
    declareHandler: (portId: string, handler: ModulePortHandler) => {
      ...
      if (
        manifestPort.required &&
        bound.length === 0 &&
        !isHostBoundRequiredInput(manifestPort)
      )
        throw new Error(
          `unbound required input port: ${module.instanceId}.${portId}`,
        );
    ...
    createParticipants: (controls) => {
      ...
      const participants = graph.constructionOrder.map((instanceId) => {
        ...
        const resolvedPorts = createResolvedPortsV13(
          ...
          (port) =>
            port.payloadType === "damage-v1" &&
            port.delivery === "event" &&
            (graphV14?.contactRoutes.some(
              (route) =>
                route.kind === "actor-root-player" &&
                route.targetSinkInstanceId === instanceId,
            ) ??
              false),
        );
        ...
        const context =
          module.factoryContextVersion === "1.2.0"
            ...
            : (() => {
                ...
                if (authority.actorSetHealth !== undefined) {
                  grantedServices.push("actorSetHealth");
                  guardedAuthority.actorSetHealth = Object.freeze({
                    damage: (routeId: string, request: Readonly<unknown>) => {
                      controls.assertService("contact-commit");
                      return authority.actorSetHealth!.damage(routeId, request);
                    },
                  });
                }
                if (authority.contactCandidates !== undefined) {
                  grantedServices.push("contactCandidates");
                  guardedAuthority.contactCandidates = Object.freeze({
                    register: (handler: (candidate: unknown) => unknown) =>
                      authority.contactCandidates!.register((candidate) =>
                        controls.injectExternalEvent(() => handler(candidate)),
                      ),
                  });
                }
                if (authority.contactCommitV2 !== undefined) {
                  grantedServices.push("contactCommitV2");
                  guardedAuthority.contactCommitV2 = Object.freeze({
                    commit: (decision: Readonly<unknown>) => {
                      controls.assertService("contact-commit");
                      return authority.contactCommitV2!.commit(decision);
                    },
                  });
                }
