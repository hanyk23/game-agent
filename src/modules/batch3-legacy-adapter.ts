...
export function adaptShooterGameSpecToBatch3(input: {
  ...
  if (
    spec.controls.touch.mode !== "drag" ||
    !spec.controls.touch.relativeMovement
  )
    throw new Batch3LegacyCoverageError(
  ...
  const enemyPatternIds = new Set(
    spec.enemyWaves.flatMap((wave) => wave.patternIds),
  );
  const bossPatternIds = new Set(
    spec.boss.phases.flatMap((phase) => phase.patternIds),
  );
  const channels = [
    ...[...enemyPatternIds].map((patternId) => ({
      role: "enemy" as const,
      patternId,
    })),
    ...[...bossPatternIds].map((patternId) => ({
      role: "boss" as const,
      patternId,
    })),
  ];
  ...
  const patternById = new Map(
    spec.bulletPatterns.map((pattern) => [pattern.id, pattern]),
  );
  ...
  const modules: any[] = [
    ...
    },
    {
      ...
      configuration: {
        device: "keyboard",
        control: spec.controls.keyboard.focus[0] ?? "ShiftLeft",
    ...
    },
    {
      ...
      versionRange: "1.1.0",
      configuration: {
        moveSpeed: spec.player.moveSpeed,
    ...
    },
    {
      ...
      configuration: {
        ...
        initialHealth: spec.player.maxHealth,
        damageFloor: 0,
      },
    },
  ];
  ...
  for (const [index, weapon] of spec.weapons.entries()) {
    ...
    modules.push(
      {
        instanceId: `${suffix}-target`,
        ownerId: "player-one",
        moduleId: "targeting.fixed-forward",
        versionRange: "1.1.0",
        configuration: { attackChannelId: channel },
      },
      {
        instanceId: `${suffix}-trigger`,
        ownerId: "player-one",
        moduleId: "trigger.interval",
        versionRange: "1.1.0",
        configuration: {
          attackChannelId: channel,
          intervalMs: weapon.fireIntervalMs,
        },
      },
      {
        instanceId: `${suffix}-delivery`,
        ownerId: "player-one",
        ...
        configuration: {
          ...
          maximumWeaponPowerBonus,
          textureRole: "player-projectile",
          spawnOffsetY: -24,
          lateralSpacing: 14,
          maxActive: Math.max(weapon.projectileCount, 16),
          maximumAcceptedRequestsPerSecond: Math.ceil(
            1000 / weapon.fireIntervalMs,
          ),
          recycleMargin: 20,
          exhaustionPolicy: "drop-and-observe",
        },
    ...
    bindings.push(
      {
        from: { instanceId: `${suffix}-target`, portId: "selection" },
        to: { instanceId: `${suffix}-delivery`, portId: "target" },
      },
      {
        from: { instanceId: `${suffix}-trigger`, portId: "request" },
        to: { instanceId: `${suffix}-delivery`, portId: "request" },
      },
    );
  }
  ...
  modules.push(
    {
      ...
      versionRange: "1.0.0",
      configuration: {
        maximumEnemies: spec.viewport.maxEnemies,
        maximumSpawnsPerSecond: Math.max(
          ...spec.enemyWaves.map((wave) =>
            Math.ceil(1000 / wave.spawnIntervalMs),
          ),
        ),
        sourceIds: spec.enemyWaves.map((wave) => wave.id),
        assetRoleBySource: Object.fromEntries(
          spec.enemyWaves.map((wave) => [wave.id, "enemy"]),
        ),
        spawnY: 0,
        radius: 16,
        waves: spec.enemyWaves.map((wave) => ({
          sourceId: wave.id,
          startsAtMs: wave.startMs,
          endsAtMs: wave.startMs + wave.durationMs,
          intervalMs: wave.spawnIntervalMs,
          maxAlive: wave.maxAlive,
          speed: wave.moveSpeed,
        })),
      },
    },
    {
      ...
      configuration: {
        bossStartMs,
        handoffId: "legacy-boss-handoff",
        maximumBosses: 1,
        sourceIds: ["boss-main"],
        assetRoleBySource: { "boss-main": "boss" },
        spawnXRatio: 0.5,
        spawnY: 96,
        radius: 32,
        horizontalSpeed: Math.max(
          ...spec.boss.phases.map((phase) => phase.moveSpeed),
        ),
        minimumXRatio: 0.1,
        maximumXRatio: 0.9,
        phases: spec.boss.phases.map((phase) => ({
          phaseId: phase.id,
          healthThreshold: phase.healthThreshold,
          patterns: phase.patternIds.map((patternId) => {
            const pattern = patternById.get(patternId)!;
            return {
              patternSourceId: patternId,
              attackChannelId: attackChannel("boss", patternId),
    ...
    },
    {
      ...
      configuration: {
        ...
        maximumHealthBySourceId: Object.fromEntries(
          spec.enemyWaves.map((wave) => [wave.id, wave.health]),
        ),
      },
    },
    {
      instanceId: "boss-health",
      ownerId: "boss-host",
      moduleId: "combat.health",
      versionRange: "1.2.0",
      configuration: {
        damageRouteId: "boss.damage",
        maximumHealthBySourceId: { "boss-main": spec.boss.maxHealth },
      },
    },
  );
  bindings.push(
    {
      from: { instanceId: "boss", portId: "handoff-request" },
      to: { instanceId: "enemy-waves", portId: "handoff-request" },
    },
    {
      from: { instanceId: "enemy-waves", portId: "handoff-cleared" },
      to: { instanceId: "boss", portId: "handoff-cleared" },
    },
    {
    ...
    {
      from: { instanceId: "boss", portId: "roots" },
      to: { instanceId: "boss-health", portId: "roots" },
    },
    {
    ...
    {
      from: { instanceId: "boss-health", portId: "health" },
      to: { instanceId: "boss", portId: "health" },
    },
    {
      from: { instanceId: "boss-health", portId: "defeated" },
      to: { instanceId: "boss", portId: "defeated" },
    },
  );
  const actorRootBindings: any[] = [
    {
      bindingId: "enemy.health",
      ...
      purpose: "health-bank",
      maximumEntries: spec.viewport.maxEnemies,
    },
    {
      bindingId: "boss.health",
      producerInstanceId: "boss",
      producerOutputPort: "roots",
      consumerInstanceId: "boss-health",
      consumerInputPort: "roots",
  ...
  const enemyTextureConsumers: string[] = ["enemy-waves"];
  const bossTextureConsumers: string[] = ["boss"];
  for (const [index, entry] of channels.entries()) {
    ...
    const allowedSourceIds =
      entry.role === "enemy"
        ? spec.enemyWaves
            .filter((wave) => wave.patternIds.includes(entry.patternId))
            .map((wave) => wave.id)
        : ["boss-main"];
    ...
    // The pattern source shares the encounter actor texture with its producer,
    // satisfying the modules' `sharing: "assembly"` asset requirement (>= 2
    // consumers) per ADR 0028 host-owned shared enemy/boss actor roots.
    (entry.role === "enemy"
      ? enemyTextureConsumers
      : bossTextureConsumers
    ).push(source);
    modules.push(
      {
        instanceId: source,
        ownerId: `${entry.role}-host`,
        moduleId: "trigger.encounter-pattern",
        versionRange: "1.1.0",
        configuration: {
          attackChannelId: channel,
          patternSourceId: entry.patternId,
          allowedSourceIds,
          activationMode:
            entry.role === "enemy" ? "root-lifecycle" : "encounter-events",
          intervalMs: pattern.intervalMs,
          durationMs: pattern.durationMs,
          maximumEmitters:
            entry.role === "enemy" ? spec.viewport.maxEnemies : 1,
        },
      },
      {
        instanceId: target,
        ...
        configuration:
          pattern.pattern === "aimed"
            ...
            : { attackChannelId: channel, direction: { x: 0, y: 1 } },
      },
      {
        instanceId: delivery,
        ownerId: `${entry.role}-host`,
        moduleId: patternDeliveryModule(pattern.pattern),
        versionRange: "1.1.0",
        configuration: deliveryConfiguration(
          pattern,
          channel,
          spec.viewport.maxEnemyBullets,
        ),
      },
      {
        instanceId: graze,
        ...
        versionRange: "1.1.0",
        configuration: {
          playerRadius: spec.player.hitboxRadius,
          bulletRadius: 4,
          margin: 12,
          ledgerCeiling: spec.viewport.maxEnemyBullets,
        },
      },
    );
    bindings.push(
      {
        ...
        to: { instanceId: source, portId: "roots" },
      },
      {
        from: { instanceId: producer, portId: "lifecycle" },
        to: { instanceId: source, portId: "lifecycle" },
      },
      ...(entry.role === "boss"
        ? [
            {
              from: { instanceId: "boss", portId: "activations" },
              to: { instanceId: source, portId: "activations" },
            },
          ]
        : []),
      {
        from: { instanceId: source, portId: "requests" },
        to: { instanceId: target, portId: "requests" },
      },
      {
        from: { instanceId: target, portId: "targeted" },
        to: { instanceId: delivery, portId: "targeted" },
      },
      {
        from: { instanceId: delivery, portId: "projectiles" },
        to: { instanceId: graze, portId: "projectiles" },
      },
    );
    actorRootBindings.push({
      bindingId: `${prefix}.source`,
      producerInstanceId: producer,
      producerOutputPort: "roots",
      consumerInstanceId: source,
      consumerInputPort: "roots",
      expectedActorRole: entry.role,
      purpose: "pattern-source",
      maximumEntries: entry.role === "enemy" ? spec.viewport.maxEnemies : 1,
    });
  }
      ...
      modules.push(
        {
          ...
          versionRange: "1.0.0",
          configuration: { maximumTrackedContacts: 10_000 },
        },
        {
          instanceId: policy,
          ownerId: target.ownerId,
          ...
          versionRange: "1.1.0",
          configuration: {
            projectileRootRouteId: target.routeId,
        ...
        },
        {
          ...
          versionRange: "1.2.0",
          configuration: { maximumResolvedContacts: 10_000 },
        },
      );
      bindings.push(
        {
          from: {
            instanceId: `weapon-${weaponIndex}-delivery`,
            portId: "projectiles",
          },
          to: { instanceId: detector, portId: "projectiles" },
        },
        {
          from: { instanceId: target.producer, portId: "roots" },
          to: { instanceId: detector, portId: "roots" },
        },
        {
          from: { instanceId: detector, portId: "candidates" },
          to: { instanceId: policy, portId: "candidates" },
        },
        {
          from: { instanceId: policy, portId: "decisions" },
          to: { instanceId: resolution, portId: "decisions" },
        },
      );
      actorRootBindings.push({
        bindingId: `${prefix}.root`,
        producerInstanceId: target.producer,
        producerOutputPort: "roots",
        consumerInstanceId: detector,
        consumerInputPort: "roots",
        expectedActorRole: target.role,
  ...
  for (const source of [
    ...
    modules.push(
      ...
      },
      {
        ...
        versionRange: "1.1.0",
        configuration: {
          projectileRootRouteId: `${source.id}.damage`,
          actorRootPlayerRouteId: "player.damage",
        },
      },
      {
    ...
    bindings.push(
      {
        from: { instanceId: source.producer, portId: "roots" },
        to: { instanceId: detector, portId: "roots" },
      },
      {
        from: { instanceId: detector, portId: "candidates" },
    ...
    actorRootBindings.push({
      ...
      producerInstanceId: source.producer,
      producerOutputPort: "roots",
      consumerInstanceId: detector,
      consumerInputPort: "roots",
      expectedActorRole: source.role,
      purpose: "body-contact",
      maximumEntries: source.maximumEntries,
    });
    actorRootMutationGrantSelections.push({
      selectionId: `${prefix}.deactivate`,
      rootBindingId,
      consumerInstanceId: detector,
      producerInstanceId: source.producer,
  ...
  modules.push(
    {
      ...
      configuration: {
        scoreBySourceId: {
          ...Object.fromEntries(
            spec.enemyWaves.map((wave) => [wave.id, wave.scoreValue]),
          ),
          "boss-main": spec.boss.scoreValue,
        },
      },
    },
    {
      instanceId: "combo-score",
      ownerId: "player-one",
      moduleId: "scoring.combo",
      versionRange: "1.0.0",
      configuration: {
        comboWindowMs: spec.scoring.comboWindowMs,
        multiplierCap: spec.scoring.comboMultiplierCap,
      },
    },
    {
      ...
      versionRange: "1.0.0",
      configuration: { award: spec.scoring.grazePoints },
    },
    {
    ...
    {
      instanceId: "win-condition",
      ownerId: "player-one",
      moduleId:
        spec.winCondition.type === "bossDefeated"
          ? "outcome.boss-defeat"
          : spec.winCondition.type === "scoreReached"
      ...
      configuration:
        spec.winCondition.type === "bossDefeated"
          ? { candidate: "win" }
          : spec.winCondition.type === "scoreReached"
            ? { candidate: "win", threshold: spec.winCondition.targetScore }
            : { candidate: "win", thresholdMs: spec.winCondition.targetMs },
    },
    {
      instanceId: "loss-condition",
      ownerId: "player-one",
      moduleId:
        spec.loseCondition.type === "timeExpired"
          ? "outcome.survival-time"
          : "outcome.player-health",
      versionRange: "1.0.0",
      configuration:
        spec.loseCondition.type === "timeExpired"
          ? { candidate: "loss", thresholdMs: spec.loseCondition.limitMs }
          : { candidate: "loss" },
    },
    {
  ...
  bindings.push(
    {
      ...
      to: { instanceId: "defeat-score", portId: "defeats" },
    },
    {
      from: { instanceId: "boss", portId: "lifecycle" },
      to: { instanceId: "defeat-score", portId: "defeats" },
    },
    {
      from: { instanceId: "defeat-score", portId: "sources" },
      to: { instanceId: "combo-score", portId: "sources" },
    },
    {
      from: { instanceId: "combo-score", portId: "transactions" },
      to: { instanceId: "score-ledger", portId: "transactions" },
    },
    ...channels.map((_, index) => ({
      from: {
        instanceId: `${channels[index]!.role}-pattern-${index}-graze`,
        portId: "graze",
      },
    ...
    ...(spec.winCondition.type === "bossDefeated"
      ? [
          {
            from: { instanceId: "boss", portId: "lifecycle" },
            to: { instanceId: "win-condition", portId: "lifecycle" },
          },
        ]
      : spec.winCondition.type === "scoreReached"
        ? [
            {
              ...
              to: { instanceId: "win-condition", portId: "score" },
            },
          ]
        : []),
    ...(spec.loseCondition.type === "healthDepleted"
      ? [
          {
            from: { instanceId: "player-health", portId: "state" },
            to: { instanceId: "loss-condition", portId: "health" },
          },
        ]
      : []),
    {
  ...
  if (modules.length > 128)
    throw new Batch3LegacyCoverageError(
      "assembly-module-limit",
      `derived module count ${modules.length} exceeds Assembly 1.3 limit 128`,
    );
  ...
  const bindAsset = (
    ...
    assetBindings.push({
      ...
      artifact,
      sharing,
      consumerInstanceIds: consumers,
      ...(approvedSharingEvidenceId === undefined
        ? {}
        : { approvedSharingEvidenceId }),
    });
  };
  for (const [index, weapon] of spec.weapons.entries())
    bindAsset(
      ...
      "player-projectile",
      "projectile",
      query(weapon.projectileAssetQueryId),
      [`weapon-${index}-delivery`],
      "instance",
    );
  bindAsset(
    "enemy",
    ...
    enemyTextureConsumers,
    "assembly",
    "legacy.texture-sharing",
  );
  bindAsset(
    "boss",
    "boss",
    "boss",
    query(spec.boss.assetQueryId),
    bossTextureConsumers,
    "assembly",
    "legacy.texture-sharing",
  );
  ...
  const assembly = GameAssemblySpecV13Schema.parse({
    ...
    assetBindings,
    globalBudget: {
      activeEntities: 10_000,
      activeProjectiles: 10_000,
      spawnsPerSecond: 10_000,
      timers: 256,
    },
    ...
    hostileAggregateBudgetGroups: [
      {
        ...
        memberInstanceIds: hostileMembers,
        activeEntityCapacity: spec.viewport.maxEnemyBullets,
        activeProjectileCapacity: spec.viewport.maxEnemyBullets,
        spawnsPerSecondCapacity: Math.min(
          10_000,
          Math.max(
            ...channels.map(({ patternId }) => {
              const pattern = patternById.get(patternId)!;
              return pattern.bulletCount * Math.ceil(1000 / pattern.intervalMs);
            }),
          ),
        ),
    ...
    actorSetDamageRoutes: [
      {
        routeId: "enemy.damage",
        rootBindingId: "enemy.health",
        orderedSinkInstanceIds: ["enemy-health"],
      },
      {
        routeId: "boss.damage",
        rootBindingId: "boss.health",
        orderedSinkInstanceIds: ["boss-health"],
      },
    ],
  ...
  const assetEvidence = Object.freeze({
    assets: Object.freeze(assetQueryEvidence.map(({ artifact }) => artifact)),
    approvedSharingEvidenceIds: Object.freeze(
      input.assets.approvedSharingEvidenceIds ?? [],
    ),
  });
  const scoreEvidence = deriveBatch3LegacyScoreEvidence({
    spec,
    sourceSpecSha256,
    assembly,
  });
  ...
  return Object.freeze({
    ...
    assembly,
    assetEvidence,
    assetQueryEvidence: Object.freeze(assetQueryEvidence),
    scoreEvidence,
  });
}
