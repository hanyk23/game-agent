...
const scrollingWavesConfiguration = z
  .strictObject({
    maximumEnemies: z.number().int().min(1).max(64),
    maximumSpawnsPerSecond: z.number().int().min(1).max(64),
      ...
      .array(
        z
          .strictObject({
            ...
            maxAlive: z.number().int().min(1).max(64),
...
const encounterPatternConfiguration = z.strictObject({
  ...
  maximumEmitters: z.number().int().min(1).max(64),
});
...
const hostileRadialConfiguration = z
  .strictObject({
    ...
    maxActive: z.number().int().min(1).max(64),
